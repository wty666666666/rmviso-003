...
namespace WINDMILL
{
    WindMill::WindMill(double time)
    {
        ...
        int x = rand() % 200 + 400;//随机坐标x
        int y = rand() % 100 + 420;//随机坐标y
        R_center = cv::Point2i(x, y);//中心坐标
    }

    void WindMill::drawR(cv::Mat &img, const cv::Point2i &center)//在图像 img 上，以 center 为中心点的位置绘制字母 “R”，颜色为红色。
    {
        cv::putText(img, "R", cv::Point2i(center.x - 5, center.y + 5), cv::FONT_HERSHEY_COMPLEX, 0.7, cv::Scalar(0, 0, 255), 2);
    }

    void WindMill::drawHitFan(cv::Mat &img, const cv::Point2i &center, double angle)//在图像 img 上绘制风车的某个叶片，该叶片以 center 为中心点，旋转 angle 度。ps:这行代码绘制锤子
    {
